...
<%
	...
	db.DBTable_Reply_Select_Id(request, "ED", "QA_Reply", ori_id);
	int index = (int)request.getAttribute("index");
	
	
	String temp;
	if(Login_id.equals(request.getAttribute("user_id"))){
		temp = "on";
...
<script type="text/javascript">
	...
	function delete_reconfirm() {	
			...
			if(Login_id == "admin"){
				...
				if(check == true){
					var pw = prompt("게시글의 비밀번호를 입력해주세요");
...
<Table style="margin-left: auto; margin-right: auto;">
	...
	<tr>
		<td align="left"><a href = "QA_Board.jsp"><input type="button" value="목록보기"></a></td>
		<td align="right" width="400">
			<form name="modify_form" action="QA_Board_Modify.jsp" method="get" onsubmit="return modify_reconfirm()">
				<input type="submit" value="글 수정">
				<input type="hidden" name="ori_id" value="<%=request.getParameter("ori_id")%>">
			</form>	
		</td>
		<td align="right">
			<form name="delete_form" action="QA_Delete" method="get" onsubmit="return delete_reconfirm()">
...
<br>
	...
	<form name="reply_form" action="QA_Reply" method="get" onsubmit="return write_reply()">
		...
		for(int i = (index-1); i >= 0; i--){
			...
			if(reply_user_id != null && reply_text !=null){
				...
				if(Login_id.equals(reply_user_id) || Login_id.equals("admin")){
					...
					out.println("<form action='QA_Reply_Delete' method='get'>");
