...
<%
	...
	db.DBConnection("ED");
	db.DBTable_Select_UserId(request, "ED", "Notice_Board", search);
	db.DBTable_Reply_Amount(request, "ED", "Notice_Reply");
	...
	for(int i = (index-1); i >= 0; i--){
		...
		out.println("<td width='300'><a href='Notice_Board_Pw.jsp?ori_id="+ori_id+"'>"+title+"</a>");
		out.println("<font color='#C6C6C6'>("+reply_amount+")</font></td>");
		out.println("<td width='100'>"+user_id+"</td></tr>");
		out.println("<hr noshade align='center' width='510' size='1' color='#EAEAEA'></hr>");
	}
	out.println("</Table>");
	out.println("<hr noshade align='center' width='510' size='1' color='#EAEAEA'></hr>");
